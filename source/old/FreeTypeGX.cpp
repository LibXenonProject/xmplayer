	...
	while (text[i]) {
		...
		if (glyphData != NULL) {
			...
			if (glyphData->glyphDataTexture) {
				...
				Menu_T(glyphData->glyphDataTexture, glyphData->glyphDataTexture->width, glyphData->glyphDataTexture->height, x_pos + RenderOffsetX + x_offset, y - RenderOffsetY + y_offset, c);
