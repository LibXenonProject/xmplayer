 ...
 */
#ifndef FREETYPEXE_H_
#define FREETYPEXE_H_
...
public:
    FreeTypeGX(FT_UInt pixelSize, uint8_t vertexIndex = 0);
    //		FreeTypeGX(FT_UInt pixelSize, uint8_t vertexIndex = GX_VTXFMT1);
    ~FreeTypeGX();

    uint16_t drawText(int16_t x, int16_t y, wchar_t *text, XeColor color, uint16_t textStyling = FTGX_NULL);
    uint16_t drawText(int16_t x, int16_t y, wchar_t const *text, XeColor color, uint16_t textStyling = FTGX_NULL);
	
	
	XenosSurface * createText(wchar_t const *text, XeColor color, uint16_t textStyling = FTGX_NULL);
