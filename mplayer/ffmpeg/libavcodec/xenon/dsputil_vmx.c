...
static void get_pixels_vmx(DCTELEM *restrict block, const uint8_t *pixels, int line_size)
{
    int i;
...
static void diff_pixels_vmx(DCTELEM *restrict block, const uint8_t *s1,
...
static void add_bytes_vmx(uint8_t *dst, uint8_t *src, int w) {
    register int i;
...
void ff_put_pixels16_vmx(uint8_t *block, const uint8_t *pixels, int line_size, int h)
{
...
void ff_avg_pixels16_vmx(uint8_t *block, const uint8_t *pixels, int line_size, int h)
{
...
static void avg_pixels8_vmx(uint8_t * block, const uint8_t * pixels, int line_size, int h)
{
    register vector unsigned char pixelsv1, pixelsv2, pixelsv, blockv;
    int i;
...
/* next one assumes that ((line_size % 8) == 0) */
static void put_pixels8_xy2_vmx(uint8_t *block, const uint8_t *pixels, int line_size, int h)
{
    register int i;
...
/* next one assumes that ((line_size % 8) == 0) */
static void put_no_rnd_pixels8_xy2_vmx(uint8_t *block, const uint8_t *pixels, int line_size, int h)
{
    register int i;
    register vector unsigned char pixelsv1, pixelsv2, pixelsavg;
    register vector unsigned char blockv, temp1, temp2;
...
/* next one assumes that ((line_size % 16) == 0) */
static void put_pixels16_xy2_vmx(uint8_t * block, const uint8_t * pixels, int line_size, int h)
{
    register int i;
...
/* next one assumes that ((line_size % 16) == 0) */
static void put_no_rnd_pixels16_xy2_vmx(uint8_t * block, const uint8_t * pixels, int line_size, int h)
{
    register int i;
    register vector unsigned char pixelsv1, pixelsv2, pixelsv3, pixelsv4;
    register vector unsigned char blockv, temp1, temp2;
...
#endif
static void vorbis_inverse_coupling_vmx(float *mag, float *ang,
                                            int blocksize)
{
    int i;
    vector float m, a;
...
/* next one assumes that ((line_size % 8) == 0) */
static void avg_pixels8_xy2_vmx(uint8_t *block, const uint8_t *pixels, int line_size, int h)
{
    register int i;
    register vector unsigned char pixelsv1, pixelsv2, pixelsavg;
...
void ff_dsputil_init_vmx(DSPContext* c, AVCodecContext *avctx)
{
...
*/
    c->diff_pixels = diff_pixels_vmx;
    c->add_bytes= add_bytes_vmx;
    if (!high_bit_depth) {
		c->get_pixels = get_pixels_vmx;
		c->clear_block = clear_block_vmx;
		c->put_pixels_tab[0][0] = ff_put_pixels16_vmx;
		/* the two functions do the same thing, so use the same code */
		c->put_no_rnd_pixels_tab[0][0] = ff_put_pixels16_vmx;
		c->avg_pixels_tab[0][0] = ff_avg_pixels16_vmx;
		c->avg_pixels_tab[1][0] = avg_pixels8_vmx;
		c->avg_pixels_tab[1][3] = avg_pixels8_xy2_vmx;
		c->put_pixels_tab[1][3] = put_pixels8_xy2_vmx;
		c->put_no_rnd_pixels_tab[1][3] = put_no_rnd_pixels8_xy2_vmx;
		c->put_pixels_tab[0][3] = put_pixels16_xy2_vmx;
		c->put_no_rnd_pixels_tab[0][3] = put_no_rnd_pixels16_xy2_vmx;
    }
...
*/
    if (CONFIG_VORBIS_DECODER)
        c->vorbis_inverse_coupling = vorbis_inverse_coupling_vmx;
