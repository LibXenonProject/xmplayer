...
int common_init(void)
{
    ...
    if (!codecs_file || !parse_codec_cfg(codecs_file)) {
        ...
        if (!parse_codec_cfg(conf_path)) {
			char config_conf[256];
			sprintf(config_conf,"%s/%s",MPLAYER_CONFDIR,"/codecs.conf");
            if (!parse_codec_cfg(config_conf)) {
...
#endif
    {
        ...
        if (font_name) {
            ...
            if (!vo_font){
				char font[256];
				sprintf(font,"%s/%s",MPLAYER_DATADIR,"/font/font.desc");
                vo_font = read_font_desc(font, font_factor, verbose>1);
			}
        }
        if (sub_font_name)
